...
export type CoJsonValue<T> = IsFunction<T> extends true ? "Functions are not allowed" : ContainsSymbolKeys<T> extends true ? "Only string or number keys are allowed" : JsonValue | CoJsonObjectWithIndex<T> | CoJsonArray<T>;
 ...
 */
export type CoJsonValue1L<T> = ExcludeEmpty<{
    [K in keyof T & string]: CoJsonValue2L<T[K]> | undefined;
}> | JsonValue;
export type CoJsonValue2L<T> = ExcludeEmpty<{
    [K in keyof T & string]: CoJsonValue3L<T[K]> | undefined;
}> | JsonValue;
export type CoJsonValue3L<T> = ExcludeEmpty<{
    [K in keyof T & string]: CoJsonValue4L<T[K]> | undefined;
}> | JsonValue;
export type CoJsonValue4L<T> = ExcludeEmpty<{
    [K in keyof T & string]: JsonValue | undefined;
}> | JsonValue;
export {};
