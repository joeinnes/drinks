...
import { RawBinaryCoStream, RawCoStreamView, } from "../coValues/coStream.js";
import { WasmCrypto } from "../crypto/WasmCrypto.js";
import { LocalNode } from "../localNode.js";
import { loadCoValueOrFail, randomAnonymousAccountAndSessionID, setupTestNode, waitFor, } from "./testUtils.js";
...
test("totalValidTransactions should return the number of valid transactions processed", async () => {
    ...
    const streamOnOtherClient = await loadCoValueOrFail(otherClient.node, stream.id);
    ...
    await waitFor(() => {
        expect(Object.keys(streamOnOtherClient.core.getCurrentContent().toJSON()).length).toEqual(2);
    });
    expect(streamOnOtherClient.core.getCurrentContent().totalValidTransactions).toEqual(2);
});
describe("isBinaryStreamEnded", () => {
    ...
    test("returns true when the last item is end", () => {
        const stream = setup();
        stream.startBinaryStream({ mimeType: "text/plain", fileName: "test.txt" }, "trusting");
...
describe("Binary stream order", () => {
    class TestStreamView extends RawCoStreamView {
        testCompareItems(a, b) {
    ...
    test("sorts by sessionID when timestamps are equal", () => {
        ...
        const itemA = {
            value: { type: "start", mimeType: "text/plain" },
            madeAt: timestamp,
            tx: { sessionID: "a", txIndex: 0 },
        };
        const itemB = {
            value: { type: "chunk", chunk: "binary_UAwECAQ" },
            madeAt: timestamp,
            tx: { sessionID: "b", txIndex: 0 },
        };
        expect(testView.testCompareItems(itemA, itemB)).toBeLessThan(0);
        expect(testView.testCompareItems(itemB, itemA)).toBeGreaterThan(0);
    });
