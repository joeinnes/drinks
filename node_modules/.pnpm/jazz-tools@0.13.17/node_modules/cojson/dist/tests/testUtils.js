...
export async function createTwoConnectedNodes(node1Role, node2Role) {
    ...
    const [node1ToNode2Peer, node2ToNode1Peer] = connectedPeers("node1ToNode2", "node2ToNode1", {
        peer1role: node2Role,
        peer2role: node1Role,
    });
...
export async function createThreeConnectedNodes(node1Role, node2Role, node3Role) {
    const [node1ToNode2Peer, node2ToNode1Peer] = connectedPeers("node1ToNode2", "node2ToNode1", {
        peer1role: node2Role,
        peer2role: node1Role,
    });
    const [node1ToNode3Peer, node3ToNode1Peer] = connectedPeers("node1ToNode3", "node3ToNode1", {
        peer1role: node3Role,
        peer2role: node1Role,
    });
    const [node2ToNode3Peer, node3ToNode2Peer] = connectedPeers("node2ToNode3", "node3ToNode2", {
        peer1role: node3Role,
        peer2role: node2Role,
    });
    const node1 = await LocalNode.withNewlyCreatedAccount({
...
export function connectTwoPeers(a, b, aRole, bRole) {
    const [aAsPeer, bAsPeer] = connectedPeers("peer:" + a.account.id, "peer:" + b.account.id, {
        peer1role: aRole,
        peer2role: bRole,
    });
...
export function shouldNotResolve(promise, ops) {
    return new Promise((resolve, reject) => {
        promise
            .then((v) => reject(new Error("Should not have resolved, but resolved to " + JSON.stringify(v))))
            .catch(reject);
...
export async function loadCoValueOrFail(node, id) {
...
export function blockMessageTypeOnOutgoingPeer(peer, messageType) {
...
export async function setupTestAccount(opts = {}) {
