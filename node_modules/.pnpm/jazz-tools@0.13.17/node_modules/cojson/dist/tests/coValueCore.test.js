...
import { createTestMetricReader, createTestNode, createTwoConnectedNodes, loadCoValueOrFail, randomAnonymousAccountAndSessionID, tearDownTestMetricReader, } from "./testUtils.js";
...
test("Can create coValue with new agent credentials and add transaction to it", () => {
    ...
    expect(coValue
        .tryAddTransactions(node.currentSessionID, [transaction], expectedNewHash, Crypto.sign(account.currentSignerSecret(), expectedNewHash))
        ._unsafeUnwrap()).toBe(true);
});
test("transactions with wrong signature are rejected", () => {
    ...
    coValue
        .tryAddTransactions(node.currentSessionID, [transaction], expectedNewHash, Crypto.sign(Crypto.getAgentSignerSecret(wrongAgent), expectedNewHash))
...
test("New transactions in a group correctly update owned values, including subscriptions", async () => {
    ...
    const manuallyAdddedTxSuccess = group.core
        .tryAddTransactions(node.currentSessionID, [resignationThatWeJustLearnedAbout], expectedNewHash, signature)
...
test("creating a coValue with a group should't trigger automatically a content creation (performance)", () => {
    ...
    const group = node.createGroup();
    const getCurrentContentSpy = vi.spyOn(CoValueCore.prototype, "getCurrentContent");
