...
import { CoList, CoMap, Group, co } from "jazz-tools";
...
describe("useCoState", () => {
    ...
    it("should load nested values if requested", async () => {
        ...
        class TestMap extends CoMap {
            constructor() {
                super(...arguments);
                this.value = co.string;
                this.nested = co.ref(TestNestedMap);
            }
        }
        const account = await createJazzTestAccount({
            isCurrentActiveAccount: true,
    ...
    it("should return null if the coValue is not found", async () => {
        ...
        const { result } = renderHook(() => useCoState(TestMap, (map.id + "123")), {
            account,
        });
    ...
    it("should update when an inner coValue is updated", async () => {
        class TestMap extends CoMap {
            constructor() {
                super(...arguments);
                this.value = co.string;
                this.nested = co.optional.ref(TestMap);
            }
        }
        const someoneElse = await createJazzTestAccount({
            isCurrentActiveAccount: true,
    ...
    it("should set the value to undefined when the id is set to undefined", () => {
        ...
        const { result, rerender } = renderHook((props) => useCoState(TestMap, props.id), {
            initialProps: { id: map.id },
        });
        expect(result.current?.value).toBe("123");
...
//# sourceMappingURL=useCoState.test.js.map