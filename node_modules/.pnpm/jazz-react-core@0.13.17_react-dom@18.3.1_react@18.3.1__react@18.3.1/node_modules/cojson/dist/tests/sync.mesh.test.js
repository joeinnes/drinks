...
import { SyncMessagesLog, blockMessageTypeOnOutgoingPeer, loadCoValueOrFail, setupTestNode, waitFor, } from "./testUtils";
function setupMesh() {
...
describe("multiple clients syncing with the a cloud-like server mesh", () => {
    ...
    test("syncs corrections from multiple peers", async () => {
        ...
        const mapOnClient = await loadCoValueOrFail(client.node, map.id);
        const mapOnCoreServer = await loadCoValueOrFail(mesh.coreServer.node, map.id);
        ...
        await waitFor(() => {
            const coValue = expectMap(mesh.edgeItaly.node.expectCoValueLoaded(map.id).getCurrentContent());
