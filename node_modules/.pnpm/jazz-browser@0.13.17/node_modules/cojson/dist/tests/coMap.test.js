...
import { hotSleep, loadCoValueOrFail, randomAnonymousAccountAndSessionID, setupTestNode, waitFor, } from "./testUtils.js";
...
test("Can get CoMap entry values at different points in time", () => {
    ...
    expect(content.atTime(beforeC).lastEditAt("hello")).toEqual(operationToEditEntry(ops[1]));
    expect(content.atTime(beforeC).nthEditAt("hello", 0)).toEqual(operationToEditEntry(ops[0]));
...
test("totalValidTransactions should return the number of valid transactions processed", async () => {
    ...
    expect(mapOnOtherClient.core.getCurrentContent().totalValidTransactions).toEqual(2);
});
