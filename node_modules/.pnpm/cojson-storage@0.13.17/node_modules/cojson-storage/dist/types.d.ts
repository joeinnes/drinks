...
export interface DBClientInterface {
    getCoValue(coValueId: RawCoID): Promise<StoredCoValueRow | undefined> | StoredCoValueRow | undefined;
    getCoValueSessions(coValueRowId: number): Promise<StoredSessionRow[]> | StoredSessionRow[];
    getSingleCoValueSession(coValueRowId: number, sessionID: SessionID): Promise<StoredSessionRow | undefined> | StoredSessionRow | undefined;
    getNewTransactionInSession(sessionRowId: number, firstNewTxIdx: number): Promise<TransactionRow[]> | TransactionRow[];
    getSignatures(sessionRowId: number, firstNewTxIdx: number): Promise<SignatureAfterRow[]> | SignatureAfterRow[];
    addCoValue(msg: CojsonInternalTypes.NewContentMessage): Promise<number> | number;
